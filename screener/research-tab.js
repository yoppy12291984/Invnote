export function mount(root){
 ...
 <details><summary>保存・試作からの引継ぎについて</summary><p>編集はこのブラウザー内で保存します。下の「現在の評価を記録」を押してください。サーバーへ投資額やメモを送信しません。</p><p>ローカル試作や別端末の記録は、元の画面で「調査記録を書き出す」→この画面で「記録を読み込む」→内容を確認して保存してください。端末間の自動同期はありません。</p><p>株価・調査資料の基準日は画面内に表示します。新規開示と週次調査は下の公開フィードに表示します。取得日・対象範囲・未精査の表示を確認してください。</p></details>
 <a href="research-lab/index.html?v=1.2" target="_blank" rel="noopener">投資調査だけを別画面で開く</a><p id="researchLoadState" role="status">投資調査を読み込み中…</p></div>
 <iframe id="researchFrame" title="投資調査：候補・反証・購入検討" src="research-lab/index.html?v=1.2" loading="lazy" style="width:100%;height:900px;display:block;border:0;border-radius:10px;background:var(--sur)"></iframe>`;
